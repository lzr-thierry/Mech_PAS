...
class MADE(nn.Module):
    def __init__(self, input_size, hidden_size, n_hidden, cond_label_size=None, activation='relu',
                 input_order='sequential', input_degrees=None, loga_bound=2.5):
        ...
        self.loga_bound = loga_bound

    @property
    def base_dist(self):
        return D.Normal(self.base_dist_mean, self.base_dist_var)

    def _m_loga(self, x, y=None):
        m, loga = self.net(self.net_input(x, y)).chunk(chunks=2, dim=1)
        # soft-clamp: ~identity near 0, saturates smoothly to ±loga_bound,
        # nonzero gradient everywhere so the net can still pull loga back
        c = self.loga_bound
        loga = c * torch.tanh(loga / c)
        return m, loga

    def forward(self, x, y=None):
        m, loga = self._m_loga(x, y)
...
def train(nf, model, optimizer, iteration, exp, log, sampling=True, update=True):
    nf.train()
    ...
    if sampling and iteration % 200 == 0:
        ...
        xkk, _ = nf(x00)
        xkk = xkk.data.numpy()
        np.savetxt(exp.output_dir + '/samples' + str(iteration), xkk, newline="\n")
    ...
    if iteration % exp.calibrate_interval == 0 and update and model.surrogate.grid_record.size(0) < exp.budget:
        ...
        print(xk0)
        model.surrogate.update(xk0, max_iters=200, store=True)
        model.surrogate.surrogate_save(iteration)
    ...
    if iteration % exp.log_interval == 0:
        ...
        torch.save(nf.state_dict(), exp.output_dir + '/flow_final.pt')
