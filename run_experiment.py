...
exp.batch_norm_order    = False                  # boo: Order to decide if batch_norm is used        default True

exp.input_size          = 42                 # int: Dimensionality of input                      default 2
exp.batch_size          = 1000                   # int: Number of samples generated                  default 100
exp.true_data_num       = 500                     # double: number of true model evaluated        default 2
exp.n_iter              = 100001                 # int: Number of iterations                         default 25001
exp.lr                  = 0.003                # float: Learning rate                              default 0.003
exp.lr_decay            = 0.9999                # float: Learning rate decay                        default 0.9999
exp.log_interval        = 100                  # int: How often to show loss stat                  default 10
exp.calibrate_interval  = 2000                   # int: How often to update surrogate model          default 1000
exp.budget              = 100500                  # int: Total number of true model evaluation
...
# model.load_state_dict(torch.load(exp.output_dir + '/flow_final.pt'))
...
for i in range(exp.n_iter):
    scheduler.step()
    train(model, rt, optimizer, i, exp, loglist, True, False) # with surrogate
...
np.savetxt(exp.output_dir + '/out_nofas.txt',
           (rt.solve_t(params)).detach().cpu().numpy())
